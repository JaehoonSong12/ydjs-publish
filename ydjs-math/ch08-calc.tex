...
\section*{Key Integration Vocabulary}

\begin{description}
  \item[Substitution (Inner/Outer) Rule] 
    Given $u = g(x)$ (the \emph{inner} function) and $f'(u)$ the \emph{outer} derivative, 
    \[
      \int f'(g(x))\,g'(x)\,dx
      = \int f'(u)\,du
      = f(u) + C.
  ...
  \item[Chain Rule]
    For the composite $f(g(x))$,
    \[
      \frac{d}{dx}\bigl[f(g(x))\bigr]
  ...
  \item[Integration by Parts]
    Rearranging the Product Rule and then integrating both sides gives
    \[
      \int U(x)\,V'(x)\,dx
      = U(x)\,V(x)
      \;-\;
      ...
      + C.
    \]
    (Think “kill” the derivative on $V'$ and “save”/promote $U$ to multiply $V$.)
\end{description}
\newpage
