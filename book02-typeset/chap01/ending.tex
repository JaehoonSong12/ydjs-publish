\cleardoublepage
\section{Summary and Tips}

\subsection*{Interesting Facts}

\begin{itemize}
  \item The word ``digit'' comes from the Latin word for finger, because people used their fingers to count.
  \item The decimal system is called ``base-10'' because each place is ten times the value of the place to its right.
  \item The largest single-digit number in the decimal system is $9$.
  \item The introduction of zero by Indian mathematicians made calculations much clearer and easier.
\end{itemize}

\subsection*{Tip for Students}

Math is not just about solving problems. It is about seeing how ideas fit together. Learn the basics---place value, fractions, comparisons, factors, primes, and multiples. They are not just chapters. They are tools. And the better you understand them, the more confidently you can use them anywhere.
