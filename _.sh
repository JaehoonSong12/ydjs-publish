...
if [[ "$SCRIPT_AUTHOR" != "Jaehoon Song" ]]; then
    echo "Error: Script author is not allowed. Exiting."
    
    # PROBLEM: return 1
    # FIX:     exit 1
    #
    # Use 'exit 1' to terminate the entire script and
    # signal an error (a non-zero exit code).
    exit 1
fi
...
# cd $TEMP[ -s "$NVM_DIR/bash_completion" ] && \. "$NVM_DIR/bash_completion"  # This loads nvm bash_completion
