\runningheader{SAT Mathematics}{Drill Set 9}{Page \thepage\ of \numpages}

\begingradingrange{sat-drill-09}
\subsection*{SAT Drill 9}

\question[1]
How many solutions does the given system of equations have?
\[
  \begin{gathered}
  y = 2x - 5 \\
  y = 2x^2 - 18x + 45
  \end{gathered}
\]
\begin{choices}
  \choice 0
  \CorrectChoice 1
  \choice 2
  \choice $\infty$
\end{choices}

\question[1]
The function $c(t) = 6.50 + 1.50t$ can be used to find the cost of an ice cream sundae, where $t$ is the number of toppings added to the sundae. If Henry ordered an ice cream sundae, paid for it with $\$20$, and received $\$6$ in change, how many toppings did Henry put on his sundae?
\begin{choices}
  \choice $3$
  \CorrectChoice $5$
  \choice $7$
  \choice $9$
\end{choices}

\question[1]
There are 3 teaspoons in a tablespoon. There are 4 tablespoons in $\frac{1}{4}$ cup. If there are 10 teaspoons of sugar in one can of soda, how many cups of sugar are in 12 cans of soda?
\begin{choices}
  \choice $\frac{5}{6}$
  \CorrectChoice $2 \frac{1}{2}$
  \choice $4$
  \choice $10$
\end{choices}

\newpage



\question[1]
Angle $R$ in a right triangle measures $n^\circ$. If $\sin n^\circ = \frac{5}{13}$, what is $\cos (90 - n)^\circ$?
\begin{solution}
$\frac{5}{13}$
\end{solution}

\question[1]
Line $l$ is perpendicular to the line $-4x + 2y = 14$ and passes through the point $(4, 6)$. What are the coordinates of the $y$-intercept of line $l$?
\begin{choices}
  \choice $(0, 4)$
  \choice $(0, 5)$
  \choice $(0, 7)$
  \CorrectChoice $(0, 8)$
\end{choices}

\question[1]
Sandra is putting together a bouquet of flowers using tulips and roses. She can have a total of 24 flowers in the bouquet. Each tulip costs $\$4$ and each rose costs $\$6$. If she spends $\$126$ on the bouquet, how many roses does Sandra put in the bouquet?
\begin{choices}
  \choice $9$
  \choice $12$
  \CorrectChoice $15$
  \choice $18$
\end{choices}

\question[1]
For a quadratic function $f(x) = x^2 + bx + c$, the value $f(3) = -4$ is the vertex of the function. What is the value of $c$?
\begin{solution}
5
\end{solution}


\newpage



\question[1]
The formula for finding the kinetic energy of an object is $K = \frac{1}{2}mv^2$, where $K$ is the kinetic energy of the object in joules, $m$ is the mass of the object in kilograms, and $v$ is the velocity of the object in meters per second. Which equation correctly expresses $v$ in terms of $K$ and $m$?
\begin{choices}
  \CorrectChoice $v = \pm \sqrt{\frac{2K}{m}}$
  \choice $v = \pm \sqrt{\frac{K}{2m}}$
  \choice $v = \pm \sqrt{\frac{2m}{K}}$
  \choice $v = \pm \sqrt{\frac{1}{2}Km}$
\end{choices}

\question[1]
The scatterplot shows the relationship between the diameter of pizza in inches at a restaurant and the price of the pizza per square inch in dollars.
\begin{center}
  \includegraphics[width=0.90\linewidth]{images/cus14}
\end{center}
Which of the following is the best estimate of the diameter, in inches, of a pizza that costs $\$0.12$ per square inch?
\begin{solutionorbox}
\end{solutionorbox}



\newpage



\question[1]
The equation of the circle in the $xy$-plane is given:
\[
  x^2 + y^2 - 6x + 12y = 3
\]
What is the center of the circle?
\begin{choices}
  \choice $(-6, 12)$
  \choice $(-3, 6)$
  \CorrectChoice $(3, -6)$
  \choice $(6, -12)$
\end{choices}

\question[1]
If $4x + 16 = 24$, what is the value of $x + 4$?
\begin{choices}
  \choice 2
  \choice 4
  \CorrectChoice 6
  \choice 8
\end{choices}

\question[1]
If $x$ is $2k\%$ of $y$, what is the value of $3k\%$ of $0.3y$?
\begin{choices}
  \choice $0.05x$
  \choice $0.2x$
  \CorrectChoice $0.45x$
  \choice $0.9x$
\end{choices}

\question[1]
For the given function $h(x) = 3x^3 - a x$, $h(2) = 20$. What is the value of $h(-1)$?
\begin{choices}
  \choice $-5$
  \CorrectChoice $-1$
  \choice $1$
  \choice $5$
\end{choices}



\newpage



\question[1]
An angle opposite a leg of a right triangle measures $w^\circ$, and $\tan(w^\circ) = \frac{3}{4}$. What is $\sin(90^\circ - w^\circ)$?
\begin{solution}
$\frac{4}{5}$
\end{solution}

\question[1]
Which of the following is equivalent to $x^{0.6} \cdot x^{\frac{3}{20}}$ for all positive values of $x$?
\begin{choices}
  \choice $\sqrt[20]{x^{0.9}}$
  \choice $\sqrt[5]{x^3}$
  \CorrectChoice $\sqrt[4]{x^3}$
  \choice $\sqrt[3]{x^4}$
\end{choices}

\question[1]
\[
  \begin{gathered}
    3.5a - 4.25b = -14.25 \\
    4.25a - 3.5b = -9
  \end{gathered}
\]
The solution to the given system of equations is $(a, b)$. What is the value of $a-b$?
\begin{choices}
  \choice $-7$
  \CorrectChoice $-3$
  \choice $5$
  \choice $7$
\end{choices}


\question[1]
A line in the $xy$-plane contains the point $(3, -5)$ and has a slope of $\frac{4}{3}$. Which of the following points lies on the line?
\begin{choices}
  \choice $\left(\frac{1}{2}, 1 \frac{2}{3}\right)$
  \choice $\left(1 \frac{2}{3}, -5\right)$
  \CorrectChoice $\left(2, -6 \frac{1}{3}\right)$
  \choice $\left(5, -5 \frac{1}{4}\right)$
\end{choices}





\newpage



\question[1]
A survey counted the number of flowers on the bushes in a park. The mean number of flowers per bush was 34 and the median number was 45. What factor most likely would account for the difference between the mean and the median?
\begin{choices}
  \CorrectChoice There were some bushes with extremely few flowers.
  \choice The mode of the data was a number greater than 34.
  \choice Many bushes had between 34 and 45 flowers.
  \choice There is little variance in the number of flowers on the bushes.
\end{choices}

\question[1]
The table shows the distribution of coffee preferences among the 27 members of the DC Book Club.
\begin{center}
  \renewcommand{\arraystretch}{1.3}
  \setlength{\tabcolsep}{0.5em}
  \captionof{table}{Coffee preferences}\label{tab:sat-drill-09-coffee}%
  \vspace{0.35em}
  \begin{adjustbox}{max width=\linewidth,clip=false,margin=0pt}
  \begin{tabular}{|c|c|c|c|}
    \cline{2-4}
    \multicolumn{1}{c|}{} &
      \cellcolor[HTML]{E0E0E0}\textbf{Cream} &
      \cellcolor[HTML]{E0E0E0}\textbf{No Cream} &
      \cellcolor[HTML]{E0E0E0}\textbf{Total} \\
    \hline
    \cellcolor[HTML]{E0E0E0}\textbf{Sugar} & 4 & 7 & 11 \\
    \hline
    \cellcolor[HTML]{E0E0E0}\textbf{No Sugar} & 15 & 1 & 16 \\
    \hline
    \cellcolor[HTML]{E0E0E0}\textbf{Total} & 19 & 8 & 27 \\
    \hline
  \end{tabular}
  \end{adjustbox}
\end{center}
If a club member is selected at random, what is the probability the member prefers no cream, given that they prefer sugar?
\begin{choices}
  \choice $\frac{7}{27}$
  \choice $\frac{7}{16}$
  \CorrectChoice $\frac{7}{11}$
  \choice $\frac{7}{8}$
\end{choices}




\newpage



\question[1]
The height of a cone is three times its diameter. The base of the cone is a circle, and the area of the base is $9\pi$ square centimeters. What is the volume of the cone, in cubic centimeters?
\begin{choices}
  \choice $27\pi$
  \CorrectChoice $54\pi$
  \choice $162\pi$
  \choice $216\pi$
\end{choices}

\question[1]
A formula used for compound interest is $A = P\left(1 + \frac{r}{n}\right)^{nt}$. Which of the following represents $P$ in terms of $A, n,$ and $t$?
\begin{choices}
  \choice $\frac{\left(1 + \frac{r}{n}\right)^{nt}}{A}$
  \choice $A\left(1 + \frac{r}{n}\right)^{nt}$
  \CorrectChoice $\frac{A}{\left(1 + \frac{r}{n}\right)^{nt}}$
  \choice $\frac{1}{A\left(1 + \frac{r}{n}\right)^{nt}}$
\end{choices}

\question[1]
A parking garage charges $\$11$ on weekdays and $\$15$ on weekends. If the garage brought in $\$53,234$ in one week and a total of 4,122 people used the garage, how many people used the garage during the weekend?
\begin{choices}
  \choice 304
  \CorrectChoice 1,973
  \choice 2,149
  \choice 3,549
\end{choices}





\newpage


\question[1]
The equation below defines a circle in the $xy$-coordinate plane:
\[
  x^2 + y^2 - 6x - 8y = -16
\]
What is the length of the circle's radius?
\begin{solution}
3
\end{solution}



\question[1]
The exponential function $f$ is defined by $f(x) = 11 \cdot b^x$ where $b$ is a positive constant. If $f(4) = 2,816$, what is the value of $f(5)$?
\begin{choices}
  \choice 4
  \CorrectChoice 11,264
  \choice 161,051
  \choice $164,916,224$
\end{choices}

\question[1]
If the solution to the given equation is $x = 4$, which of the following could be a value of $a$ and $b$?
\[
  a(x + 4) = b(x - 1) + 1
\]
\begin{choices}
  \choice $a = 8$ and $b = -3$
  \choice $a = 7$ and $b = 6$
  \choice $a = 4$ and $b = -5$
  \CorrectChoice $a = -4$ and $b = -11$
\end{choices}


\newpage



\question[1]
If $\frac{a^{2x^3}}{a^4} = a^{12}$, what is the value of $2^x$?
\begin{solution}
4
\end{solution}

\question[1]
If $(3x + a)(2x + b) = 6x^2 + c x + 6$ and $4a + 6b = 36$ for all values of $x$, what is the value of $c$?
\begin{solution}
18
\end{solution}

\question[1]
The graph of line $q$ is perpendicular to the line $2x - 4y = 11$ and contains the point $(1, 6)$. Line $p$ contains the points $\left(2 \frac{1}{4}, 7 \frac{1}{4}\right)$ and $\left(\frac{1}{3}, 5 \frac{1}{3}\right)$. If $p$ and $q$ intersect at the point $(a, b)$, what is the value of $a - b$?
\begin{choices}
  \CorrectChoice $-5$
  \choice $1$
  \choice $5$
  \choice $6$
\end{choices}



\newpage





\question[1]
In triangles $ABC$ and $DEF$, $\angle A$ and $\angle D$ each measure $48^\circ$, and the measures of $\overline{AB}$ and $\overline{DE}$ are both 5 inches. Which of the following is sufficient to prove that $ABC$ and $DEF$ are congruent triangles?
\begin{choices}
  \choice $\overline{BC}$ measures 3 inches and $\overline{EF}$ measures 4 inches.
  \choice $\overline{BC}$ measures 4 inches and $\overline{EF}$ measures 4 inches.
  \CorrectChoice $\overline{AC}$ measures 4 inches and $\overline{DF}$ measures 4 inches.
  \choice $\overline{BC}$ measures 3 inches and $\overline{DF}$ measures 4 inches.
\end{choices}

\question[1]
\[
  f(x) = -2x^2 + 16x - 11
\]
If $g(x) = f(x - 3)$, what is the value of $x$ where $g(x)$ reaches its maximum value?
\begin{solution}
7
\end{solution}





\newpage



\question[1]
Which expression is equivalent to $\frac{25(x^2 - 5)}{(x - \sqrt{5})}$, where $x \neq \sqrt{5}$?
\begin{choices}
  \choice $5(x - 5)$
  \choice $5(x + 5)$
  \choice $25(x - \sqrt{5})$
  \CorrectChoice $25(x + \sqrt{5})$
\end{choices}

\question[1]
A sequence of numbers starts at the number $a$. Each sequential number is $k$ more than the previous number. The sum of the first 13 numbers can be written as $xa + yk$. What is the value of $x + y$?
\begin{solution}
91
\end{solution}

\question[1]
Maximum Mail Order Company upgrades its computers each year. The function $P(t)=36 t+410$ models the company's budget for upgrades, in thousands of dollars, $t$ years after 2004. What is the best interpretation of 36 in this context?
\begin{choices}
  \choice The number of years until the budget is completely spent
  \choice The amount budgeted, in thousands of dollars, in the year 2004
  \CorrectChoice The annual increase, in thousands of dollars, in the budget each year after 2004
  \choice The number of years in which the budget will increase after 2004
\end{choices}




\newpage





\question[1]
Which expression is equivalent to $\frac{4 x^2-3}{2 x+\sqrt{3}}$?
\begin{choices}
  \choice $2x-3$
  \CorrectChoice $2x-\sqrt{3}$
  \choice $2x+\sqrt{3}$
  \choice $2x+3$
\end{choices}

\question[1]
What is the $y$-intercept for the given equation?
\[
  y-7=3^x-5
\]
\begin{solution}
3
\end{solution}

\question[1]
A linear model estimates that the mean weight of a calf in New Zealand, at birth, is 34 kilograms. At 180 days after birth, the mean weight is 272 kilograms. Which function best defines this model, where $f(x)$ is the mean weight of a calf, in kilograms, $x$ days after it is born, for $0 \leq x \leq 180$?
\begin{choices}
  \choice $f(x)=0.76 x+34$
  \choice $f(x)=0.76 x+180$
  \CorrectChoice $f(x)=1.32 x+34$
  \choice $f(x)=34 x+272$
\end{choices}




\newpage





\question[1]
For the function $f$, the table shows some values of $x$ and their corresponding values of $f(x)$. If $f(1)=t$, what is the value of $f(3t)$?
\begin{center}
  \renewcommand{\arraystretch}{1.3}
  \setlength{\tabcolsep}{0.5em}
  \captionof{table}{$x$ and $f(x)$}\label{tab:sat-drill-09-f-sample}%
  \vspace{0.35em}
  \begin{adjustbox}{max width=\linewidth,clip=false,margin=0pt}
  \begin{tabular}{|c|c|}
    \hline
    \cellcolor[HTML]{E0E0E0}{$x$} & \cellcolor[HTML]{E0E0E0}{$f(x)$} \\
    \hline
    1 & 3 \\
    \hline
    3 & 6 \\
    \hline
    5 & 9 \\
    \hline
    7 & 12 \\
    \hline
    9 & 15 \\
    \hline
  \end{tabular}
  \end{adjustbox}
\end{center}
\begin{solution}
15
\end{solution}

\question[1]
The chart shows the average cost of housing, in dollars per square foot, in Nebraska from 2015 to 2018. By approximately what percent did the average cost of housing increase over this period?
\begin{center}
  \includegraphics[width=0.90\linewidth]{images/cus15}
\end{center}
\begin{choices}
  \choice $20\%$
  \choice $28\%$
  \choice $39\%$
  \choice $47\%$
\end{choices}



\newpage






\question[1]
The given function shows the number of bacteria in a pond after $t$ days since the bacteria began to populate the pond.
\[
  p(t)=2034(3)^{\frac{t}{14}}
\]
Which of the following is the best interpretation of $(3)^{\frac{t}{14}}$ in the equation?
\begin{choices}
  \choice The number of bacteria at the beginning of the study
  \CorrectChoice The number of bacteria triples every two weeks
  \choice The number of bacteria increases by 3 every two weeks
  \choice The number of bacteria in the pond after two weeks
\end{choices}

\question[1]
\[
  12 x^4-8 x^3-15 x^2
\]
The given expression is equal to $x^2(q x+5)(r x-3)$, where $q>0$ and $r>0$. What is the value of $q+r$?
\begin{solution}
8
\end{solution}

\question[1]
Line $a$ is parallel to line $b$. Line $a$ goes through the points $(-4,0)$ and $(0,6)$. If line $b$ goes through the point $(-6,0)$, what is the equation for line $b$?
\begin{choices}
  \choice $y=\frac{2}{3} x-6$
  \choice $y=\frac{2}{3} x+4$
  \choice $y=\frac{3}{2} x+6$
  \CorrectChoice $y=\frac{3}{2} x+9$
\end{choices}




\newpage





\question[1]
The exterior angle of an equilateral triangle is equal to $(8x-20)^\circ$. What is the value of $x$?
\begin{choices}
  \choice 5
  \choice 10
  \choice 12.5
  \CorrectChoice 17.5
\end{choices}

\question[1]
How many solutions does the given equation have?
\[
  (x-1)(x+2)=x(x-3)
\]
\begin{choices}
  \choice 0
  \CorrectChoice 1
  \choice 2
  \choice 4
\end{choices}

\question[1]
Which of the following coordinates would be true for the given system of inequalities?
\[
  \begin{gathered}
  y>-2x-1 \\
  3y<x+9
  \end{gathered}
\]
\begin{choices}
  \choice $(-3,3)$
  \choice $(-2,1)$
  \choice $(1,4)$
  \CorrectChoice $(3,1)$
\end{choices}



\newpage






\question[1]
If the given system of equations has infinite solutions, what is the value of $b$?
\[
  \begin{aligned}
  9x-14y &= -3 \\
  ax-by &= 6
  \end{aligned}
\]
\begin{solution}
-28
\end{solution}

\question[1]
Which of the following equivalent forms of the given function $f$ displays the $x$-intercepts as constants or coefficients?
\[
  f(x)=6x^2-45x+75
\]
\begin{choices}
  \choice $f(x)=(2x-5)(3x-15)$
  \choice $f(x)=3x(2x-15)+75$
  \choice $f(x)=6\left(x-\frac{15}{4}\right)^2+\frac{75}{8}$
  \CorrectChoice $f(x)=6(x-5)\left(x-\frac{5}{2}\right)$
\end{choices}

\question[1]
In the given system of equations, $n$ is a positive constant. For what value of $n$ does the system have exactly one real solution?
\[
  \begin{gathered}
  y=4 \\
  y=x^2+12x+n
  \end{gathered}
\]
\begin{solution}
40
\end{solution}




\newpage





\question[1]
If $(a, b)$ is a solution to the given system of inequalities, what is the minimum value of $b$?
\[
  \begin{gathered}
  y \geq 2(x-3)^2+5 \\
  y \leq x+3
  \end{gathered}
\]
\begin{choices}
  \choice $-5$
  \choice $-3$
  \choice $3$
  \CorrectChoice $5$
\end{choices}

\question[1]
Biology students grew a culture of bacteria at a constant temperature of $55^\circ\mathrm{C}$ for 3 days. The students began with 200 spores and the number of spores doubled every 12 hours. Of the equations given, which can be used to correctly calculate the number of spores in the bacteria culture at the end of 3 days?
\begin{choices}
  \choice $y=200(12)^3$
  \choice $y=200(2)^3$
  \CorrectChoice $y=200(2)^6$
  \choice $y=200(12)^2$
\end{choices}




\newpage





\question[1]
Which of the statements best compares the means and standard deviations of Data Set $J$ and Data Set $K$?
\begin{center}
  \begin{adjustbox}{max width=\linewidth,center}
    \includegraphics[width=0.88\linewidth]{images/cus16}
  \end{adjustbox}
\end{center}
\begin{choices}
  \choice The mean of Data Set $J$ is less than the mean of Data Set $K$, and the standard deviations of the two sets are equal.
  \choice The mean of Data Set $J$ is greater than the mean of Data Set $K$, and the standard deviations of the two sets are equal.
  \choice The means of the two sets are equal, and the standard deviation of Data Set $J$ is less than the standard deviation of Data Set $K$.
  \choice The means of the two sets are equal, and the standard deviation of Data Set $J$ is greater than the standard deviation of Data Set $K$.
\end{choices}




\newpage





\question[1]
What is the circumference of the circle with an equation of $x^2+6x+y^2-4y=51$?
\begin{choices}
  \choice $8\pi$
  \choice $14.28\pi$
  \CorrectChoice $16\pi$
  \choice $64\pi$
\end{choices}

\question[1]
In the triangle shown, $\angle ABC=90^\circ$. Line segment $BD$ is shown with point $D$ lying on line $AC$.
\begin{center}
  \includegraphics[width=0.90\linewidth]{images/cus17}
\end{center}
What is the value of $\sin \angle ABD-\cos \angle DBC$?
\begin{choices}
  \choice 0
  \choice 30
  \choice 45
  \choice 90
\end{choices}

\question[1]
In the system of equations shown, $k$ is an integer constant. What is the largest value of $k$ for which the system has no solution?
\[
  \begin{gathered}
  y=(x-6)(x+6) \\
  y=k
  \end{gathered}
\]
\begin{solution}
-37
\end{solution}




\newpage





\question[1]
The base of the exponential function $g(x)$ is $k$, where $k$ is a positive real number, and $k$ is equivalent to the value of the $y$-coordinate of the $y$-intercept of the graph of $g(x)$. Which of the following functions describes the exponential function $g(x-1)$?
\begin{choices}
  \CorrectChoice $g(x-1)=k^x$
  \choice $g(x-1)=k^{x+1}$
  \choice $g(x-1)=k^x+k$
  \choice $g(x-1)=k^{x-1}+k$
\end{choices}

\endgradingrange{sat-drill-09}
