...
\ifPDFTeX
  ...
  % Define Korean font family - use a more standard approach
  \newCJKfamily{kor}{NanumGothic}
  \newCJKfamily{koralt}{NanumMyeongjo}
  
  % Usage for pdfLaTeX: \kr{...} wraps Korean text using a CJK family code.
  \newenvironment{foreigntext}[1]{\begin{CJK}{UTF8}{#1}}{\end{CJK}}
  \newcommand{\kr}[1]{\begin{foreigntext}{kor}#1\end{foreigntext}}
  
  % Alternative command for different font style
  \newcommand{\krserif}[1]{\begin{foreigntext}{koralt}#1\end{foreigntext}}
\else
  ...
  % Try to set Korean fonts with fallbacks
