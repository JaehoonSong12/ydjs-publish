...
\begin{define}{Uniqueness of Limits}[lem:uniqueness-of-limits][red]
  ...
  \begin{subprove}
    ...
    \[
      \abs{s_n - L_1} < \varepsilon \quad\text{for all } n \geq N_1,
    \]
    and
    \[
      \abs{s_n - L_2} < \varepsilon \quad\text{for all } n \geq N_2.
    \]
    Let \(N = \max(N_1, N_2)\). Then for all \(n \geq N\),
    \[
      \abs{L_1 - L_2} = \abs{(L_1 - s_n) + (s_n - L_2)} \leq \abs{L_1 - s_n} + \abs{s_n - L_2} < 2\varepsilon = \frac{2\abs{L_1 - L_2}}{3},
    \]
    that is, \(\abs{L_1 - L_2} < \frac{2\abs{L_1 - L_2}}{3}\), which is a contradiction.
...
\begin{enumerate}
  ...
  \begin{subprove}
    ...
    Let
    \[
      M = \max\left\{ \abs{s_1}, \abs{s_2}, \ldots, \abs{s_{N-1}}, 1 + \abs{s} \right\}.
    \]
...
\begin{define}{Cauchy Sequence}[def:cauchy-sequence][red]
  ...
  \[
    \abs{s_n - s_m} < \varepsilon \quad\text{for all } n, m \geq N.
  \]
\end{define}
...
\begin{enumerate}
  ...
  \begin{subprove}
    ...
    \[
      \abs{s_n - s_m} < \varepsilon \quad\text{for all } n, m \geq N.
    \]
    Let \(\varepsilon = \dfrac{1}{2}\) and $n=m+1$. Then,
    \begin{align*}
      \abs{s_n - s_m} &= \abs{(-1)^{m+2}\dfrac{m+1}{m+2} - (-1)^{m+1}\dfrac{m}{m+1}} \\
      &= \abs{\dfrac{m+1}{m+2} + \dfrac{m}{m+1}} 
      \geq \dfrac{2(m+1)}{m+2}.
    \end{align*}
    Since \(m \geq N \geq 1\), it follows that
    \[
      \dfrac{2(m+1)}{m+2} \geq \dfrac{1}{2}.
    \]
    which is a contradiction.
    Therefore, by \nameref{def:cauchy-sequence}, the sequence \(s_n=(-1)^{n+1}\dfrac{n}{n+1}\) is not Cauchy.
    By \nameref{def:convergence-of-a-sequence}, the sequence is divergent.
  \end{subprove}
...
\begin{enumerate}
  ...
  \begin{subprove}
    ...
    \[
      \max\left\{s_{n-1}, L - \frac{1}{n}\right\} < s_n < L.
    \]
  ...
  \begin{enumerate}
    ...
    \begin{subprove}
      ...
      \]
      Therefore, by definition, \(\lim s_n = s\).
    \end{subprove}
