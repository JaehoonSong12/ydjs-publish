...
\end{choices}
\begin{solution}
  \lstinline[style=bashinline]{cd} to the parent folder, then scaffold:
\begin{lstlisting}[style=bash]
ng new my-app
\end{lstlisting}
  \lstinline[style=bashinline]{ng serve} runs a dev server for an \emph{existing} project---not for creating one.
\end{solution}
...
ng serve --open
\end{lstlisting}
  Default URL: \texttt{http://localhost:4200/}. \lstinline[style=bashinline]{--open} launches the browser. \lstinline[style=bashinline]{ng help} lists commands. (Older quickstarts used \lstinline[style=bashinline]{npm start} on port \texttt{3000}.)
\end{solution}
...
\end{lstlisting}
  The displayed text updates when \texttt{title} changes in the class. Interpolation syntax is double curly braces: \lstinline[style=htmlinline]|{{ }}|.
\end{solution}
...
\begin{choices}
  \CorrectChoice \lstinline[style=htmlinline]|{{ }}|
  \choice \lstinline[style=htmlinline]|[]|
  \choice \lstinline[style=htmlinline]|{}|
  \choice \lstinline[style=htmlinline]|[()]|
\end{choices}
\begin{solution}
  \lstinline[style=htmlinline]|{{ expression }}| evaluates an expression from the component and inserts the result into the DOM. \lstinline[style=htmlinline]|[prop]="expr"| is property binding; \lstinline[style=htmlinline]|[(ngModel)]| is two-way binding.
\end{solution}
...
\begin{solution}
  ...
  \lstinline[style=htmlinline]|[(ngModel)]="propertyName"| ---combining property binding and event binding in one notation.
\end{solution}
...
\begin{choices}
  \choice \lstinline[style=htmlinline]|<input [value]="value">| only
  \CorrectChoice \lstinline[style=htmlinline]|<input [(ngModel)]="value">| and \lstinline[style=htmlinline]|{{ value }}|
  \choice \lstinline[style=htmlinline]|{{ ngModel }}| only in the TypeScript file
  \choice \lstinline[style=htmlinline]|<button [click]="clearValue">| without parentheses
\end{choices}
\begin{solution}
\begin{lstlisting}[style=htmlinline]
...
\begin{choices}
  ...
  \choice Using \lstinline[style=htmlinline]|[(...)]| syntax (for example \lstinline[style=htmlinline]|[(ngModel)]|)
  \CorrectChoice Both A and B
  \choice None of the above
\end{choices}
\begin{solution}
  \textbf{Both.} You can combine one-way bindings manually (listen to \lstinline[style=htmlinline]|(input)| and bind \lstinline[style=htmlinline]|[value]|), or use \lstinline[style=htmlinline]|[(ngModel)]| as shorthand. \lstinline[style=htmlinline]|[(ngModel)]| is the usual approach for form fields.
\end{solution}
...
\begin{solution}
  ...
  \textbf{Two-way:} \lstinline[style=htmlinline]|[(ngModel)]| (or equivalent property + event pair) keeps input and class property synchronized---useful for live form feedback such as password rules.
\end{solution}
...
\end{lstlisting}
  Use \lstinline[style=htmlinline]|*ngIf="condition; else elseBlock"| with \lstinline[style=htmlinline]{<ng-template \#elseBlock>} for an alternate block. Common uses: show content by user role, age gates, feature flags.
\end{solution}
...
\end{choices}
\begin{solution}
  Backend data often arrives in a raw shape (ISO dates, plain numbers). Pipes format values \emph{in the view} without changing the underlying model. Examples: \lstinline[style=htmlinline]|date|, \lstinline[style=htmlinline]|currency|, \lstinline[style=htmlinline]|lowercase|, \lstinline[style=htmlinline]|percent|.
\end{solution}
...
\end{choices}
\begin{solution}
  Interpolation uses \lstinline[style=htmlinline]|{{ }}|; the vertical bar (\texttt{|}) separates the value from the pipe name. Optional parameters follow a colon: \lstinline[style=htmlinline]!{{ today | date:'MM-dd-yyyy' }}!.
\end{solution}
...
\end{choices}
\begin{solution}
  \textbf{Parent $\rightarrow$ child:} bind a property with \lstinline[style=htmlinline]|[inputProp]="parentValue"| on the child selector (backed by \lstinline[style=tsinline]|@Input()| in the child class).
...
\begin{choices}
  ...
  \choice Set \lstinline[style=htmlinline]|ViewEncapsulation.None|
\end{choices}
\begin{solution}
...
\begin{choices}
  ...
  \choice \lstinline[style=htmlinline]|async| replaces \texttt{HttpClientModule}
\end{choices}
\begin{solution}
  Define \lstinline[style=tsinline]|heroes$ = this.service.getHeroes();| in the class, then bind \lstinline[style=htmlinline]!{{ heroes$ | async }}! in HTML. The pipe handles subscribe/unsubscribe for that binding.
\end{solution}
...
\end{choices}
\begin{solution}
  The router updates the browser URL and swaps components in a \lstinline[style=htmlinline]|<router-outlet>|. Users bookmark \texttt{/products/42}; the app loads the correct feature view client-side.
\end{solution}
...
\begin{choices}
  \choice \lstinline[style=htmlinline]|<a href="/heroes">Heroes</a>| only (always full page reload in every setup)
  \CorrectChoice \lstinline[style=htmlinline]|<a routerLink="/heroes">Heroes</a>| (with \lstinline[style=htmlinline]|RouterModule| imported)
  \choice \lstinline[style=htmlinline]|<a ngModel="/heroes">Heroes</a>|
  \choice \lstinline[style=htmlinline]|<a *ngFor="/heroes">Heroes</a>|
\end{choices}
\begin{solution}
  \lstinline[style=htmlinline]|routerLink| (and \lstinline[style=htmlinline]|routerLinkActive| for active CSS) delegates navigation to the Angular router. Plain \lstinline[style=htmlinline]|href| may reload the entire document unless configured for hash routing or server fallback.
\end{solution}
...
\end{choices}
\begin{solution}
  Parent layouts keep chrome (nav bar); child routes render inside the outlet. Nested routes use multiple outlets or child \lstinline[style=htmlinline]|<router-outlet>| elements in feature components.
\end{solution}
