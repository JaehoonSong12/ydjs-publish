...
% Optional/stream-pipeline FRQs migrated from \texttt{cs-a/03-practice.tex} open the \textbf{Stream API} section (before stream MCQs); parallel execution via \texttt{parallelStream()} ties to fork/join pools in \texttt{distributed\_computing.tex}.
...
\uplevel{\par\textbf{Optional, streams, and parallel execution}\par}
