...
\end{choices}
\begin{solution}
  A correct skeleton starts with the HTML5 doctype, then \texttt{<html>}, then separate \texttt{<head>} (metadata) and \texttt{<body>} (visible content) sections---the required top-level trio is \texttt{html}, \texttt{head}, and \texttt{body}. Elements such as \texttt{footer} may appear inside \texttt{body} but are not required siblings of \texttt{head}. Reversing \texttt{head} and \texttt{body} or omitting the doctype breaks the standard document tree.
\begin{lstlisting}[style=htmlinline]
<!DOCTYPE html>
<html lang="en">
  <head><title>Page Title</title></head>
...
\end{choices}
\begin{solution}
  The \texttt{<html>} element wraps the entire document. \texttt{<head>} stores metadata (title, meta tags, stylesheet links), while \texttt{<body>} contains what users see in the rendered page.
\begin{lstlisting}[style=htmlinline]
<head>
  <meta charset="utf-8">
...
\end{choices}
\begin{solution}
  Hyperlinks are created with \texttt{<a>} (anchor), not \texttt{<link>}---\texttt{<link>} attaches external resources such as stylesheets inside \texttt{<head>}. The \textbf{hypertext reference} attribute \texttt{href} on \texttt{<a>} points to the destination URL or path. Presentation is CSS; \texttt{target} and JS handlers affect navigation behavior separately.
\begin{lstlisting}[style=htmlinline]
...
\end{choices}
\begin{solution}
  Semantic elements describe roles (navigation, main content, aside) rather than being purely presentational wrappers. \textbf{HTML5} added these landmarks so crawlers and developers can infer structure; \texttt{<div>} is non-semantic until given a role or ARIA.
\begin{lstlisting}[style=htmlinline]
...
\end{choices}
\begin{solution}
  Semantic landmarks describe page regions explicitly for search engines, assistive technologies, and maintainability. \textbf{HTML5} (2014) introduced elements such as \texttt{<nav>}, \texttt{<article>}, and \texttt{<section>} so structure and grouping carry meaning without relying solely on generic \texttt{<div>} wrappers. Semantic tags describe \emph{meaning} (\emph{this is navigation}, \emph{this is an article}), improving readability and SEO; styling still requires CSS. Use \texttt{<nav>} for major navigation blocks---not \texttt{<header>} alone, \texttt{<section>}, or \texttt{<article>}.
\begin{lstlisting}[style=htmlinline]
...
\end{choices}
\begin{solution}
  Pair \texttt{for} on the label with \texttt{id} on the input so screen readers announce the field and clicking the label text focuses the control. HTML5 validation attributes such as \lstinline[style=htmlinline]|required| enforce that a field must be filled before submit (distinct from \lstinline[style=htmlinline]|action|/\lstinline[style=htmlinline]|method| on the form or the default \lstinline[style=htmlinline]|value|).
\begin{lstlisting}[style=htmlinline]
...
\end{choices}
\begin{solution}
  Block elements such as \texttt{<div>} begin on a new line and stretch to the container width. Inline elements (\texttt{<span>}, \texttt{<a>}, \texttt{<strong>}) flow with surrounding text; \texttt{<img>} is replaced inline but participates in line layout differently from block boxes.
\begin{lstlisting}[style=htmlinline]
...
\end{choices}
\begin{solution}
  HTML is a markup language for structure and semantics. \textbf{JavaScript} runs in the browser and adds client-side interactivity---DOM updates, event handlers, and API calls---while CSS handles presentation and server-side languages manage databases. Logic, loops, and calculations belong in JavaScript (or another programming language), not in HTML.
\begin{lstlisting}[style=htmlinline]
...
\end{choices}
\begin{solution}
  The browser parses HTML into a tree rooted at \texttt{<html>}. JavaScript reads and mutates nodes to update the page without a full reload. The same runtime may persist data in the browser: \lstinline[style=jsinline]|localStorage| keeps key--value strings for the origin until cleared (unlike tab-scoped \lstinline[style=jsinline]|sessionStorage|).
\begin{lstlisting}[style=htmlinline]
...
\end{choices}
\begin{solution}
  \textbf{Cascading Style Sheets (CSS)} is the language for styling web pages; HTML defines structure and JavaScript adds behavior (SQL queries databases). CSS can be applied three ways: \textbf{inline} (\texttt{style} attribute), \textbf{embedded} (\texttt{<style>} block), and \textbf{external} (\texttt{<link rel="stylesheet">}). Inline styles sit on one element and win the normal cascade.
\begin{lstlisting}[style=htmlinline]
<head>
...
\end{choices}
\begin{solution}
  The canonical box model layers are \textbf{content}, \textbf{padding}, \textbf{border}, and \textbf{margin} (inside to outside). ``Whitespace'' is not a separate layer---visible spacing comes from margin and padding. \lstinline[style=cssinline]|box-sizing: border-box| includes padding and border in the declared width; the default \lstinline[style=cssinline]|content-box| adds them outside the content width.
\begin{lstlisting}[style=htmlinline]
...
\end{choices}
\begin{solution}
  An \textbf{ID selector} (prefix \texttt{\#}) matches the one element on the page with that \texttt{id}. Class selectors use a dot (\texttt{.class}), type selectors use the tag name, and the universal selector \lstinline[style=cssinline]|*| matches every element. Each \texttt{id} must be unique within the document (duplicate ids are \emph{invalid HTML}, but browsers still render the page rather than throwing a compile-time error). Duplicates break predictable behavior: \lstinline[style=jsinline]|document.getElementById()| and \lstinline[style=jsinline]|querySelector('#id')| return only the first match, and \lstinline[style=htmlinline]|<label for="id">| can associate with at most one control. IDs carry higher specificity than classes but should not replace classes for broad styling.
\begin{lstlisting}[style=cssinline]
...
\end{checkboxes}
\begin{solution}
  \textbf{Flexbox} arranges items along a single main axis (one-dimensional); \textbf{CSS Grid} handles rows and columns together (two-dimensional). Float and table layouts are older patterns. \texttt{justify-content} distributes flex items along the main axis. \texttt{align-items} and \texttt{align-content} work on the cross axis; \texttt{flex-direction} sets the main axis direction but does not align items along it.
\begin{lstlisting}[style=css]
.toolbar {
  display: flex;
...
\end{checkboxes}
\begin{solution}
  Mobile-first starts with a minimal layout and adds complexity with \texttt{min-width} \textbf{media queries}, keeping default payloads light for constrained devices. \textbf{Media queries} apply styles when viewport width, orientation, or user preferences match a condition---Flexbox helps layout but does not by itself switch rules at breakpoints.
\begin{lstlisting}[style=css]
...
\end{choices}
\begin{solution}
  \lstinline[style=jsinline]|var| is function-scoped and leaks out of blocks. \lstinline[style=jsinline]|let| and \lstinline[style=jsinline]|const| are block-scoped (ES6). \lstinline[style=jsinline]|static| is a class member modifier, not a variable declaration keyword; \lstinline[style=jsinline]|define| is not a JavaScript keyword.
\begin{lstlisting}[style=js]
if (true) {
  var framework = "React";
  let library = "Zustand";
...
\end{choices}
\begin{solution}
  In JavaScript, \lstinline[style=jsinline]|this| refers to the current execution context---often the object before the dot when a method is called. Arrow functions do not bind their own \lstinline[style=jsinline]|this|; they capture the enclosing scope's \lstinline[style=jsinline]|this|. Regular functions bind \lstinline[style=jsinline]|this| at call time.
\begin{lstlisting}[style=js]
const user = {
  name: "Jaehoon",
...
\end{choices}
\begin{solution}
  An \lstinline[style=jsinline]|async| function wraps return values in a resolved promise and thrown errors in a rejected promise, even when no explicit \lstinline[style=jsinline]|return| is written. Under the hood, \textbf{Promises} are the core abstraction for async work (\emph{pending}, \emph{fulfilled}, \emph{rejected}); callbacks predate them, and \lstinline[style=jsinline]|async|/\lstinline[style=jsinline]|await| is syntactic sugar on top of Promises.
\begin{lstlisting}[style=js]
...
\end{choices}
\begin{solution}
  \lstinline[style=jsinline]|.map()| transforms each element into a new array of values; \lstinline[style=jsinline]|.filter()| selects a subset; \lstinline[style=jsinline]|.reduce()| accumulates to one value; \lstinline[style=jsinline]|.forEach()| runs side effects without returning a mapped array. \lstinline[style=jsinline]|.map()| and \lstinline[style=jsinline]|.filter()| return new arrays and leave the source unchanged. Mutating methods include \lstinline[style=jsinline]|.push()|, \lstinline[style=jsinline]|.pop()|, \lstinline[style=jsinline]|.splice()|, and \lstinline[style=jsinline]|.sort()|.
\begin{lstlisting}[style=js]
