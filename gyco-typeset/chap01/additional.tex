...
\blockquote{무작정 외우지 말고 문제 읽을때 한번 뜻을 생각해보면 이해가 됨}
...
\blockquote{prime factorization(소인수분해) 말그대로 prime(소수)까지 분해한다는 거임}

\blockquote{제일 기본적인 순서는 소인수분해할 숫자를 보는거임}

\blockquote{그 숫자가 몇의 배수인지 봐야하는데 이건}
...
\blockquote{일단 대충 저걸로 판단후 최소 값부터 계속 소인수분해 (ex: 60을 소인수 분해 하면 일단 2로 나눈다 왜냐하면 짝수이기 때문; 2*30; 30도 짝수이기 때문에 2로 다시 나눈다; 2*2*15; 15는 일의 자리가 5이기 때문에 5로 나눈다; 2*2*5*3; 와 다 소수네; 소인수분해 완료)}

\subsection{Finding GCF and LCM}

The greatest common factor (GCF) and least common multiple (LCM) are essential concepts for working with multiple numbers. While these ideas may seem complex, there's a simple method using the Korean "ㄴ" technique that makes finding them straightforward.

\blockquote{한국 학교에서 당연히 배웠겠지만 'ㄴ'자로 하면됨}

\blockquote{'ㄴ'자 방법이란 소인수분해랑 똑같은건데 두수 혹은 그 이상의 숫자들이 공유하는 약수로 다 나눈건데 이거는 꼭 소수로 안해도 됨}
...
\blockquote{위에처럼 마지막 숫자가 소수가 되면 끝}

\blockquote{최대공약수의 경우 'ㄴ'자 옆에 있는 숫자들의 곱; 지금 같은 경우는 6}
...
\blockquote{최소공배수의 경우 'ㄴ'자 옆밑에 있는 모든 숫자들의 솝; 지금 같은 경우는 6*2*3 = 36}
