...
\kr{5104에서 0은 자릿수 유지만을 위해 존재한다 십의 자리가 비어 있음을 보여주는 중요한 역할 숫자 위치를 유지해 주는 자리 표시자}
...
\kr{자리값은 숫자의 위치에 따라 달라짐 왼쪽으로 한 자리 이동할 때마다 그 10배 예시: 10의 자리는 1의 자리보다 10배 큼}
...
\kr{소수랑 분수는 같은거임 예시: 0.25 = 25/100 = 1/4}
...
\kr{비교, 비교, 질문, 고뇌 비교후 고뇌를 통해 뭐가 큰지 찾으세요}
...
\kr{기호 설명 덧붙이기: <, <=, >, >= greater than (>), greater than or equal to (>=), less than (<), less than or equal to(<=)}
...
\kr{factor = 약수; 어떤 수나 식을 나누어 나머지가 없이 떨어지는 수나 식을 일컫는 말 예시: 3은 6의 약수이다; 6/3=2}

\kr{multiple = 배수; 어떤 특정 정수에 다른 정수를 곱해서 얻어지는 수 예시: 6은 3의 배수이다; 3*2=6}
...
\kr{prime number = 소수; 어떤 특정 정수에 다른 정수를 곱해서 얻어지는 수 예시: 2, 3, 5, 7, 11, etc}

\kr{composite number = 합성수; 1보다 큰 자연수 중 소수가 아닌 수 예시: 10(5*2) , 35(7*5), 40(8*5)}
...
\kr{GCF = 최대공약수; 1보다 큰 자연수 중에서 1과 자기 자신만을 약수로 갖는 소수가 아닌 수 예시: 12와 18의 최대공약수는 6 (ㄴ 예시)}

\kr{LCM = 최소공배수; 두 개 이상의 여러 수에 공통으로 존재하는 배수들 중에서 가장 작은 수 예시: 12와 18의 최소공배수는 36 (ㄴ 예시)}
