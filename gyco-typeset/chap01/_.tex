...
\blockquote{In \(5104\), the \(0\) adds nothing but still matters.}

\kr{5104에서 0은 값을 더하지 않지만 중요한 역할을 한다. 십의 자리가 비어 있음을 보여주며 숫자의 위치를 유지하는 자리 표시자이다.}
...
\blockquote{Each place in a number is ten times the value of the one to its right.}

\kr{각 자리값은 바로 오른쪽 자리의 10배이다. 예: 십의 자리는 일의 자리보다 10배 크다.}

\subsection{What is Place Value? / \kr{자리값이란?}}
...
\newpage
%%%%%%% concept02.tex
\section{Decimals and Fractions / \kr{소수와 분수}}
...
\kr{소수와 분수는 서로 다른 언어가 아니라 서로 변환할 수 있는 표현 방식이다. 예: \(0.75 = \tfrac{3}{4}\).}


\subsection{What are Decimals and Fractions? / \kr{소수와 분수란?}}
...
\kr{분수에는 진분수, 가분수, 대분수가 있다. 진분수는 $\dfrac{1}{4}, \dfrac{2}{4}, \dfrac{3}{4}$ 와 같이 분자가 분모보다 작은 분수다. 가분수는 $\dfrac{4}{4}, \dfrac{5}{4}$와 같이 분자가 분모와 같거나 분모보다 큰 분수이고 대분수는 $1 \dfrac{1}{4}, 2 \dfrac{3}{4}$과 같이 자연수와 진분수로 이루어진 분수다. 대분수 $1 \dfrac{1}{4}$을 가분수로 나타내면 $\dfrac{5}{4}$ 이다. 가분수 $\dfrac{9}{4}$를 대분수로 나타내면 $2 \dfrac{1}{4}$ 이다.}
...
\kr{단위분수는 $\dfrac{1}{2}, \dfrac{1}{3}, \dfrac{1}{4}$ 같이 분자가 1인 분수이고, 진분수의 한 종류이다. 단위분수 $\times$ 단위분수는 분자는 그대로 두고, 분모끼리 곱하여 계산한다. 진분수 $\times$ 진분수는 분자는 분자끼리, 분모는 분모끼리 곱하여 계산한다. 이때 약분이 되면 약분을 해서 기약분수로 나타낸다.}
...
\newpage
%%%%%%%% concept03.tex
\section{Comparing and Ordering Numbers / \kr{수의 비교와 정렬}}
...
\kr{수학은 단순히 수를 아는 것에서 끝나지 않고, 수 사이의 관계를 이해하는 것이다.}

\blockquote{We compare. We rank. We ask, which is bigger? Which is smaller?}

\kr{수를 비교한다. 순서를 정한다. 뭐가 크고, 뭐가 작은지 묻는다.}

\blockquote{Symbols like \(>\), \(<\), and \(=\) help us answer.}

\kr{>, <, = 기호는 수의 크기를 비교하는 데 사용된다.}

\subsection{How do we compare and order numbers? / \kr{어떻게 비교하고 정렬하나요?}}
...
\newpage
%%%%%%%% concept04.tex
\section{Factors and Multiples / \kr{약수와 배수}}
...
\kr{factor = 약수. 약수는 어떤 수를 나누었을 때 나머지가 없이 나누어 떨어지게 하는 수이다. 예: 3은 6의 약수이다 6/3=2
}

\kr{multiple = 배수. 배수는 어떤 정수에 다른 정수를 곱하여 얻은 수이다. ​​예: 6은 3의 배수이다 3*2=6
}

\subsection{What are Factors and Multiples? / \kr{약수와 배수란?}}
...
\paragraph{Common Denominators / 통분}
두 분모의 곱을 이용하거나, 최소공배수를 이용해서 분모를 같게 만드는 것을 통분이라고 한다.
...
\newpage
% concept05.tex
\section{Prime and Composite Numbers / \kr{소수와 합성수}}
...
\kr{prime number = 소수. 소수는 1과 자기 자신 두 개의 약수만 가지는 수이다. 예: 2, 3, 5, 7.}

\kr{composite number = 합성수. 합성수는 두 개보다 많은 약수를 가지는 수이다. 예: 10, 15, 20.}

\subsection{What are Prime and Composite Numbers? / \kr{소수와 합성수란?}}
...
\newpage
%%%%%%%% concept06.tex
\section{Greatest Common Factor \& Least Common Multiple / kr{최대공약수와 최소공배수}}
...
\kr{GCF = 최대공약수. 1보다 큰 자연수 중에서 1과 자기 자신만을 약수로 갖는 소수가 아닌 수. 예시: 12와 18의 최대공약수는 6}

\kr{LCM = 최소공배수. 두 개 이상의 여러 수에 공통으로 존재하는 배수들 중에서 가장 작은 수 예시: 12와 18의 최소공배수는 36}

\subsection{What are GCF and LCM? / \kr{최대공약수와 최소공배수란?}}
...
\newpage
%%%%%%%% ending.tex
\section{Tip for Students / 유용한 팁}
