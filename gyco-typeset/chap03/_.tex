...
\kr{율은 모든 비교를 같은 기준으로 만들어 줌 예: 가격을 비교할 때 $/개 단위로 계산하면 어떤 상품이 더 저렴한지 알 수 있음}

\blockquote{A rate is a special type of ratio that compares two different kinds of
units.}

\kr{rate(율)은 특별한 종류의 비로, 서로 다른 단위를 비교함 예시: 100km ÷ 2시간 = 시속 50km}
...
\kr{단위율은 공정한 비교를 가능하게 해 줌 예시: 사과 가격을 개당으로, 거리 속도를 1시간당으로 비교}
...
\kr{unit rate(단위율)은 두 번째 수가 1이 되도록 계산한 율 예시: 노트 3권이 $6일 때, 권당 가격은 $2 (3:6 → 1:2)}
...
\kr{비례식은 cross multiplication(교차 곱셈)으로 풀 수 있음 예: x/5 = 6/10 → 10x = 30 → x = 3}
...
\kr{proportion(비례식)은 두 비가 같음을 나타냄 예: 2/3 = 4/6}
...
\kr{분수를 백분율로 바꾸려면 100을 곱하면 됨 예: 1/4 = 0.25 → 25%}

\blockquote{A percent shows part of a whole, out of \(100\). It is a number that
means ``per 100.''}

\kr{percent(백분율)은 '100분의 ~'를 의미 예: 50% = 50/100 = 1/2}
